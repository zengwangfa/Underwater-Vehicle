...
=======
		...
		/* 【推进器调整方向系数】*/
//		Parameter_SelfCheck( (uint32 *)&Direction.UP_P1,  &Normal_Parameter[DIRECTION_UP_P1_e] );		//左标志位前进系数
//		Parameter_SelfCheck( (uint32 *)&Direction.UP_P2,  &Normal_Parameter[DIRECTION_UP_P2_e] );		//右标志位前进系数
//		Parameter_SelfCheck( (uint32 *)&Direction.DOWN_P1,&Normal_Parameter[DIRECTION_DOWN_P1_e] );		//左标志位后退系数
//		Parameter_SelfCheck( (uint32 *)&Direction.DOWN_P2,&Normal_Parameter[DIRECTION_DOWN_P2_e] );		//右标志位后退系数
//		Parameter_SelfCheck( (uint32 *)&Direction.LEFT_P, &Normal_Parameter[DIRECTION_LEFT_P1_e] );		//向左系数
//		Parameter_SelfCheck( (uint32 *)&Direction.RIGHT_P,&Normal_Parameter[DIRECTION_RIGHT_P1_e] );	//向右系数
//		
...
<<<<<<< HEAD
		ef_port_write(Nor_FLASH_ADDRESS+4*BATTERY_CAPACITY_e ,(uint32 *)&Sensor.PowerSource.Capacity,4); //深度传感器 类型
=======
		ef_port_write(Nor_FLASH_ADDRESS+4*BATTERY_CAPACITY_e ,(uint32 *)&Sensor.PowerSource.Capacity,4); //电池容量参数 3s/4s/6s
		
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_UP_P1_e ,   (uint32 *)&Direction.UP_P1,4);		//左标志位前进系数
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_UP_P2_e ,   (uint32 *)&Direction.UP_P2,4);		//右标志位前进系数
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_DOWN_P1_e , (uint32 *)&Direction.DOWN_P1,4);	//左标志位后退系数
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_DOWN_P2_e , (uint32 *)&Direction.DOWN_P2,4);	//右标志位后退系数
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_LEFT_P1_e , (uint32 *)&Direction.LEFT_P,4);		//向左系数
		ef_port_write(Nor_FLASH_ADDRESS+4*DIRECTION_RIGHT_P1_e ,(uint32 *)&Direction.RIGHT_P,4);	//向右系数
...
;	  log_i("rightUp_Dir               %d",PropellerDir.rightUp);
	  ...
	  log_i("leftMiddle_Dir            %d",PropellerDir.leftMiddle);
		...
		log_i("----------------------   ---------");
		log_i("Battery Capacity          %d",Sensor.PowerSource.Capacity);//电池容量参数
...
{
		if(*TempParameter <10000 && (int)*TempParameter > -500)	//粗糙判断,检测flash值为大致正确值
		{
