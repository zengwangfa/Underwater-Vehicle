...
void Normal_Parameter_SelfCheck_With_Flash(void) //Flash参数自检 若为 0 则为 非正常数据 
{
    ...
    Parameter_SelfCheck( (uint32 *)&YunTai.MedValue,&Normal_Parameter[YUNTAI_MED_VALUE_e] );  
		...
		Parameter_SelfCheck( (uint32 *)&PropellerDir.leftUp     ,&Normal_Parameter[PROPELLER_LEFT_DOWN_DIR_e] );// 左下  【推进器方向 参数】	
		...
		/* 【推进器动力系数】  */
		Parameter_SelfCheck( (uint32 *)&Power,&Normal_Parameter[PROPELLER_POWER_e] );// 右中  【推进器方向 参数】	
}
/*
void test_env(void) {
...
//------------------------------------------------------------------------------------------//
		...
		ef_port_write(Nor_FLASH_ADDRESS+4*PROPELLER_POWER_e ,(uint32 *)&Power,4); //右中
}	
...
void list_value(void)
{
    ...
    log_i("----------------------   ---------");
	  ...
	  log_i("leftMiddle_Dir            %d",PropellerDir.leftMiddle);
		...
		log_i("Propeller_Power           %d",Power);//推进器动力系数
